(function(w){
  ...
  document.addEventListener('DOMContentLoaded',function(){
    ...
    if(!tabs||!D)return;
    var ids=['nova','studyflow','aura','logocraft','scamshield','webcut','luxzy','yip','sunora'];
    ...
    function select(key){
      ...
      preview.onclick=function(){if(visual){visual.animate([{transform:'scale(1)'},{transform:'scale(1.045)'},{transform:'scale(1)'}],{duration:700,easing:'cubic-bezier(.22,.8,.2,1)'});}if(typeof w.openCase==='function')w.openCase(key)};
    }
